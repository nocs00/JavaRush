package com.javarush.test.level34.lesson02.home01.my_buggy_solution;
...
//        solution.recursion(s,0);
    ...
    private String findFimpleExpression(String s) {
        ...
        for (int i = 0; i < rpn.size(); i++) {
            ...
            if (isOperator(symbol)) {
                simple = findRightVariant(rpn.get(i-2)+symbol+rpn.get(i-1), s);
                break;
            }
            if (isFunc(symbol)) {
    ...
    private String findRightVariant(String simpleEx, String fullEx) {
        List<String> symbols = getSymbols(simpleEx);
        String variant1 = "(" + symbols.get(0) + ")" + symbols.get(1) + symbols.get(2);
        String variant2 = symbols.get(0) + symbols.get(1) + "(" + symbols.get(2) + ")";

        if (fullEx.contains(simpleEx)) return simpleEx;
        else if (fullEx.contains(variant1)) return variant1;
        else if (fullEx.contains(variant2)) return variant2;
...
//        if ((exp.length() > 3) && (exp.startsWith("(") && exp.endsWith(")"))) exp = exp.substring(1, exp.length()-1);
    ...
    List<String> getReversePolishNotation(final String expression) {
        ...
        for (String symbol : symbols) {
            if (isOperand(symbol) ||
                    (symbol.startsWith("-")
                            && isOperand(symbol.substring(1, symbol.length())))) {
                output.add(symbol);
            }
...
//                if (isOperand(current.substring(1, current.length()))) continue;
//
//            }
